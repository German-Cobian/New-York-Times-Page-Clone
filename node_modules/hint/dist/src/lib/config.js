...
const loadPackageJSONConfigFile = (filePath) => {
    ...
    try {
        return utils_fs_1.loadJSONFile(filePath).hintConfig || null;
    }
    catch (e) {
...
const composeConfig = (userConfig, parentConfig = '') => {
    ...
    const configurations = userConfig.extends.map((config) => {
        ...
        loadedConfiguration.hints = utils_1.normalizeHints(loadedConfiguration.hints);
...
class Configuration {
        ...
        switch (path.extname(filePath)) {
            ...
            case '.json':
                if (path.basename(filePath) === 'package.json') {
                    ...
                    config = utils_fs_1.loadJSONFile(filePath);
                }
                break;
            case '.js':
                config = utils_fs_1.loadJSFile(filePath);
                break;
            default:
                config = null;
        }
    ...
    static fromConfig(config, options) {
        ...
        const hints = Configuration.cleanHints(utils_1.normalizeHints(userConfig.hints));
    ...
    static validateConnectorConfig(config) {
        ...
        const Connector = utils_1.loadResource(connectorId, utils_1.ResourceType.connector);
        debug(`Connector schema:`);
        debug(Connector.schema);
        debug(`User configuration:`);
        debug(config.connector.options);
        return utils_json_1.validate(Connector.schema, config.connector.options).valid;
    }
